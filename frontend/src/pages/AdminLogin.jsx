...
const AdminLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error(err);
      setError(
        err.response?.data?.message || 'Authentication failed. Please verify your admin credentials.'
      );
    } finally {
      setLoading(false);
    }
    ...
    <div className="min-h-[75vh] flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 relative bg-[#f8fafc]">
      ...
      >
        <div className="text-center mb-8">
          <div className="inline-flex p-3 bg-red-50 rounded-2xl text-[#DA2824] mb-4">
            <ShieldCheck size={28} />
          </div>
          <h2 className="text-2xl md:text-3xl font-extrabold text-[#DA2824] heading-display tracking-tight">
            Admin Portal Access
          </h2>
          <p className="text-slate-500 text-sm mt-2 leading-relaxed">
            Enter authorized administrator credentials to manage NewsHub operations.
          </p>
        </div>
